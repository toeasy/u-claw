...
OPENCLAW_DIR="$UCLAW_DIR/app/core"
...
ARCH=$(uname -m)
if [ "$ARCH" = "arm64" ]; then
    NODE_DIR="$UCLAW_DIR/app/runtime/node-mac-arm64"
else
    NODE_DIR="$UCLAW_DIR/app/runtime/node-mac-x64"
fi
...
mkdir -p "$STATE_DIR" "$DATA_DIR/memory" "$DATA_DIR/backups"
...
check_status() {
    ...
    [ -f "$CONFIG_PATH" ] && CONFIG_READY="yes"
}
...
show_menu() {
    ...
    [ "$CONFIG_READY" = "yes" ] && STATUS_PORTABLE="${GREEN}已配置${NC}"
...
start_gateway() {
    ...
    for i in $(seq 1 30); do
        ...
        if curl -s -o /dev/null http://127.0.0.1:18789/ 2>/dev/null; then
            local TOKEN
            TOKEN=$(python3 -c "import json; d=json.load(open('$CONFIG_PATH')); print(d.get('gateway',{}).get('auth',{}).get('token',''))" 2>/dev/null)
